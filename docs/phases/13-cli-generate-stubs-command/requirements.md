...
phase_objective: Add a first-class `pycro generate_stubs` CLI command that regenerates project-local `pycro.pyi` using the canonical API metadata pipeline.

## Acceptance Criteria

- `pycro generate_stubs` is defined as a supported CLI command.
- The command regenerates `pycro.pyi` (project-local default path) from canonical API metadata rather than from handwritten stub content.
