...
fn main() {
	...
	if target_os == "ios" {
		...
		// bindgen/clang does not accept Rust's shortened `*-apple-ios-sim` triple.
		// Force a clang-compatible simulator triple when building iOS simulator targets.
		if target.ends_with("-apple-ios-sim") {
			...
			args.push(format!("--target={clang_arch}-apple-ios-simulator"));
		}
