    ...
    if #[cfg(all(feature = "compile-time-rng", any(test, fuzzing)))] {
        ...
        fn get_fixed_seeds() -> &'static [[u64; 4]; 2] {
            ...
            SEEDS.get_or_init(|| {
                ...
                if getrandom::fill(&mut result).is_err() {
                    // On constrained wasm targets, runtime RNG may be unavailable.
                    // Fall back to deterministic constants instead of panicking.
                    return Box::new([PI, PI2]);
                }
